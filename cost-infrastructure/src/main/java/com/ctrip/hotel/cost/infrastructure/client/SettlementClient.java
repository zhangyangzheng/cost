...
public class SettlementClient {
    ...
    // 现付订单抛到前置模块
    public boolean callSettlementPayDataReceive(SettlementPayDataReceiveDto settlementPayDataReceiveDto) throws Exception {
        boolean result = true;
        ...
        if (response != null
            ...
            if (response.getSettlementPayDataResultList().get(0) != null
                    ...
                    != 0) {
                throw new Exception("settlementPayDataReceive client error, msg: " + response.getSettlementPayDataResultList().get(0).getMessage());
            }
        }
        return result;
    }
    ...
    // 抛结算新增修改流程
    public boolean callSettlementApplyList(SettlementApplyListDto settlementApplyListDto) throws Exception {
        boolean result = true;
        ...
        if (response != null
            ...
            if (DefaultValueHelper.getValue(response.getSettleDatas().get(0).getRetCode()) != 0) {
                throw new Exception("settlementApplyList client error, msg: " + response.getSettleDatas().get(0).getMessage());
            }
        } else {
    ...
    public boolean callCancelOrder(CancelOrderDto cancelOrderDto) throws Exception {
        ...
        if (response != null
            ...
            if (response.getResponseResult().getStatus() != null
                ...
                result = false;
                LogHelper.logError(ThreadLocalCostHolder.getTTL().get().getLinkTracing(), "cancelorder client error, response is fail", ThreadLocalCostHolder.getTTL().get().getTags());
            }
        }
        return result;
    }

    public boolean callSettlementCancelList(SettlementCancelListDto settlementCancelListDto) throws Exception {
        boolean result = true;
        ...
        if (response != null && !ListHelper.isEmpty(response.getSettleDatas())) {
            if (response.getSettleDatas().get(0) != null
                ...
                throw new Exception("settlementCancelList client error, msg: " + response.getSettleDatas().get(0).getMessage());
            }
        } else {
